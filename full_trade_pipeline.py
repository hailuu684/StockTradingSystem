...
This module is intentionally additive: copy it into your repo root, for example:
    ...
    Stocks/full_trade_pipeline_checked.py
...
CLI examples
------------
python full_trade_pipeline_checked.py --write-default-config ./configs/trade_pipeline_checked.json
python full_trade_pipeline_checked.py --mode initial --config ./configs/trade_pipeline_checked.json
python full_trade_pipeline_checked.py --mode daily_news --config ./configs/trade_pipeline_checked.json
"""
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "project_name": "vnstock_ms_style_checked_pipeline",
    "paths": {
        "repo_root": ".",
        "output_dir": "./outputs/trade_pipeline_checked",
    ...
    "agent": {
        "enabled": True,
        "provider": "heuristic",
        "api_key_env": "TRADING_LLM_API_KEY",
        "api_key": None,
        "endpoint_url": None,
        "model": "trading-analyzer",
        "temperature": 0.2,
    ...
    "daily_news": {
        "enabled": True,
        "watchlist_path": "./outputs/trade_pipeline_checked/watchlist_latest.csv",
        "lookback_days": 3,
...
class AgentTradeAnalyzer:
    ...
    def analyze(self, analysis: SymbolAnalysis, rag_context: str = "", mode: str = "initial") -> Dict[str, Any]:
        ...
        if not bool(get_in(self.cfg, "agent.enabled", True)) or provider == "heuristic":
            return self._heuristic_decision(analysis, mode=mode)
        if provider == "openai_compatible":
            try:
                return self._call_openai_compatible(prompt)
            except Exception as exc:
                logging.warning("Agent API failed; falling back to heuristic: %s", exc)
                return self._heuristic_decision(analysis, mode=mode)
        logging.warning("Unsupported agent provider=%s; falling back to heuristic.", provider)
        return self._heuristic_decision(analysis, mode=mode)
...
class FullTradePipeline:
    def __init__(self, config: Optional[JsonLike] = None):
    ...
    def _init_paths(self) -> Dict[str, Path]:
        root = ensure_dir(get_in(self.cfg, "paths.output_dir", "./outputs/trade_pipeline_checked"))
        out = {
            "root": root,
    ...
    def run_initial(self) -> Dict[str, Any]:
        ...
        if bool(get_in(self.cfg, "reports.enabled", True)):
            ...
            report_path = self.paths["reports"] / f"reports_{now_stamp()}.txt"
    ...
    def _analyze_one_symbol(
        ...
        a.decision = self.agent.analyze(a, rag_context=self.rag_context, mode="initial")
        a.research_markdown = self.markdown.build_symbol_pack(a, universe_row)
        return a
    ...
    def run_daily_news(self, watchlist_path: Optional[Union[str, Path]] = None) -> Dict[str, Any]:
        ...
        for _, row in watch.iterrows():
            ...
            pseudo.decision = self.agent.analyze(pseudo, rag_context=self.rag_context, mode="daily_news")
    ...
    def _build_initial_report(
        ...
        if top.empty:
            ...
            lines.append("symbol | action | sector | composite | expected_return | risk_reward | reason")
            lines.append("-" * 118)
            for _, r in top.iterrows():
                reason = str(r.get("reason", "")).replace("\n", " ")[:90]
                lines.append(
                    f"{str(r.get('symbol', '')):6s} | "
                    f"{str(r.get('action', '')):13s} | "
                    f"{str(r.get('sector', '')):10s} | "
                    f"{as_numeric(r.get('composite')):8.2f} | "
                    f"{as_numeric(r.get('expected_return')):14.2%} | "
                    f"{as_numeric(r.get('risk_reward')):10.2f} | "
                    f"{reason}"
                )
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Checked full Vnstock trade pipeline")
    parser.add_argument("--mode", choices=["initial", "daily_news"], default="initial")
    parser.add_argument("--config", default="./configs/trade_pipeline.json")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    if args.write_default_config:
        ...
        return
    pipe = FullTradePipeline(args.config)
